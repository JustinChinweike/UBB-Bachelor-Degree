...
void UI::printUserMenu() {
    ...
    std::cout << "3. Save Shopping Basket To File" << std::endl;
    std::cout << "4. Purchase all trench coats in the shopping cart" << std::endl;
    std::cout << "5. Display The Shopping Basket with the correct application" << std::endl;
...
void UI::addTrenchCoat()
{
    ...
    //Validation
    try {
        TrenchCoatValidator::validateID(id);
        TrenchCoatValidator::validateSize(sizeOfTrench);
        TrenchCoatValidator::validateColour(colourOfTrench);
        TrenchCoatValidator::validatePrice(priceOfTrench);
        TrenchCoatValidator::validateQuantity(quantityOfTrench);
        TrenchCoatValidator::validatePhotographyLink(photography_linkOfTrench);
    }
    catch (const ValidatorException& e) {
        std::cerr << e.what() << endl;
        return; // Abrot adding the trech coat 
    }
...
void UI::updateTrenchCoat()
{
    ...
    // Validation
    try {
        TrenchCoatValidator::validateID(newID);
        TrenchCoatValidator::validateSize(sizeOfTrench);
        TrenchCoatValidator::validateColour(colourOfTrench);
        TrenchCoatValidator::validatePrice(priceOfTrench);
        TrenchCoatValidator::validateQuantity(quantityOfTrench);
        ...
        std::cerr << e.what() << endl;
        return; // Abort updating the trench coat 
...
void UI::displayShoppingBasket()
{
    cout << "Displaying the shopping basket....." << endl;

    //Ask user to enter the filename of the shopping basket
    string filename;
    cout << "Enter the filename of the shopping basket (with .csv or .html extension): ";
    cin >> filename;
    ...
    if (extension == "csv") {
        //For CSV files, open with Notepad or other text editors
        command = "start excel.exe  " + filename;
    }
    else if (extension == "html") {
        //For HTML files, open with my default web browser 
        command = "start " + filename; // This opens the file with my default web browser
    }
    else {
...
void UI::saveShoppingBasketToFile(const std::vector<TrenchCoat>& shoppingBasket)
{
    cout << "Choose the file format to save the shopping basket" << endl;
    cout << "1. CSV" << endl;
    cout << "2. HTML" << endl;
    cout << ">";
    ...
    switch (choice) {
    case 1:
        saveShoppingBasketToCSV(shoppingBasket);
        break;
    case 2:
        saveShoppingBasketToHTML(shoppingBasket);
        break;
    default:
...
void UI::saveShoppingBasketToCSV(const std::vector<TrenchCoat>& shoppingBasket) {
    std::string filename;
    std::cout << "Enter the filename to save the shopping basket (with .csv extension): ";
    std::cin >> filename;
    try {
        FileStorage::saveToCSV(shoppingBasket, filename);
        std::cout << "Shopping basket saved to " << filename << std::endl;
    }
...
void UI::saveShoppingBasketToHTML(const std::vector<TrenchCoat>& shoppingBasket) {
    std::string filename;
    std::cout << "Enter the filename to save the shopping basket (with .html extension): ";
    std::cin >> filename;
    try {
        FileStorage::saveToHTML(shoppingBasket, filename);
        std::cout << "Shopping basket saved to " << filename << std::endl;
    }
    catch (const RepositoryException& e) {
        std::cerr << e.what() << std::endl;
...
void UI::userMode() {
    ...
    while (command < 7 && command > 0)
    {
        ...
        case 2:
            ...
            break;
        case 3:
            this->saveShoppingBasketToFile(this->basket_controller.getAllTrenchCoats());
            break;
        case 4:
