...
void testRemoveAllOccurrences() {
	cout << "Test RemoveAllOcuurences" << endl;
	Bag b;
	b.add(5);
	b.add(1);
	b.add(5);
	...
	b.add(9);
	assert(b.size() == 8);
	assert(b.removeAllOccurences(5) == 4); // Remove all occurrences of 5
	assert(b.size() == 4); // Only 4 elements left after removal
	assert(b.nrOccurrences(5) == 0); // No occurrences of 5 left in the bag
}
