...
void testIsSubsetOf() {
	cout << "Test isSubsetOf" << endl;
	SortedSet s1([](TComp e1, TComp e2) { return e1 < e2; });
	SortedSet s2([](TComp e1, TComp e2) { return e1 < e2; });
	...
	// Add an element to the first set that is not in the second set
	s1.add(20);

	// Now the first set should not be a subset of the second set
	assert(s1.isSubsetOf(s2) == false);
}
