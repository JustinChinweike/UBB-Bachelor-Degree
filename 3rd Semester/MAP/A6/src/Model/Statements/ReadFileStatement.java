...
public class ReadFileStatement implements IStatement {
    ...
    public MyIDictionary<String, IType> typecheck(MyIDictionary<String, IType> typeEnv)
            throws StatementException , DataStructureExceptions{

        IType variableType = typeEnv.lookup(variableName);
        IType valueExpression = expression.typecheck(typeEnv) ;
        if (variableType instanceof IntType) {

            if(valueExpression instanceof StringType)
                return typeEnv;
            else {
                throw new StatementException("Expression is not a string type: ");
            }
        }
        else
            throw  new StatementException(" Expression variable is not of type int");
    }
